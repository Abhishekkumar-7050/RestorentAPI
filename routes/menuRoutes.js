...
// to get menuitem by the  ingredient
router.get("/:tasteType", async (req, res) => {
  try {
    const ingredintype = req.params.tasteType;
    if (
      ingredintype === "sweet" ||
      ingredintype === "spicy" ||
      ingredintype === "sour"
    ) {
      const respons = await MenuItem.find({ taste: ingredintype });
      return res.status(200).send(respons);
    } else {
      return res.status(500).send("this taste is not Present");
    }
  } catch (error) {
    console.log(error);
